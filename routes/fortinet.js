/*
This page will create and manage all the required APIs related to FortiNet
Home, Customer KPI, RealTime, Maps, Campaign for Wifi Analytics related menu/pages are managed 
*/
    ...
    sqlRequest.query('select * from userAuthenticationTable', function(err, recordset) {
        ...
        for (var attributename in recordset) {
            if (recordset[attributename].EmailId == req.body.emailId) {
                userExist = true;
                auth = {
                    'user': recordset[attributename].WifiUserName,
                    'pass': recordset[attributename].WifiPassword
                };
            }
        }
        if (userExist) {
...
// customer kpi -> Persons In & Out
router.post('/inOut/getInOut/:planType', function(req, res) {

    request({
        url: configDetails.BASEURL_FORTINET + 'inOut/getInOut/' + req.params.planType,
        proxy: configDetails.PROXY,
        method: 'post',
        auth: auth,
        json: req.body
...
/**
 * Campaign APIs - Create and manage campaigns for Forninet offers
 * Total 11 APIs  had been created to manage campaign offers 
 */
...
// To get campaign results - daily summary - execution details
router.get('/campaignResult/:range/:tgr/:id/:date', function(req, res) {
    request({
        url: configDetails.BASEURL_FORTINET + 'campaignResult/' + req.params.range + '/' + req.params.tgr + '/' + req.params.id + '/' + req.params.date,
        proxy: configDetails.PROXY,
        method: 'get',
        auth: auth
    }, function(error, response, body) {
...
// To get Number of views, Number of logins and Acceptance Rate for the Social Wifi Logins
router.get('/socialWifi/successRatePer/:range', function(req, res) {
    request({
        url: configDetails.BASEURL_FORTINET + 'socialWifi/successRatePer/' + req.params.range,
        proxy: configDetails.PROXY,
        method: 'get',
        auth: auth
    }, function(error, response, body) {
...
//to get the Account list for Social Wifi logins list
router.post('/accountsList/filter/:stDate/:endDate/:searchKey/:loginType/:next/:pageNumber', function(req, res) {
    request({
        url: configDetails.BASEURL_FORTINET + 'accountsList/filter/' + req.params.stDate + '/' + req.params.endDate + '/' + req.params.searchKey+ '/' + req.params.loginType+ '/' + req.params.next+ '/' + req.params.pageNumber,
        proxy: configDetails.PROXY,
        method: 'post',
        auth: auth
    }, function(error, response, body) {
...
/* This is a common function to send the error*/
...
// If fortinet client url gets changed then below code will throw this error message
